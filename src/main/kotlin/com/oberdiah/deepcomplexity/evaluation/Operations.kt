...
enum class UnaryNumberOp {
    ...
    fun <T : Number> applyToExpr(expr: Expr<T>): Expr<T> = when (this) {
        PLUS -> expr
        NEGATE -> NegateExpr.new(expr)
        INCREMENT, DECREMENT -> ArithmeticExpr.new(
            expr,
    ...
    companion object {
        fun fromJavaTokenType(tokenType: IElementType): UnaryNumberOp? = when (tokenType) {
...
enum class BinaryNumberOp {
    ...
    companion object {
        fun fromJavaTokenType(tokenType: IElementType): BinaryNumberOp? = when (tokenType) {
...
enum class BooleanOp {
    ...
    companion object {
        fun fromJavaTokenType(tokenType: IElementType): BooleanOp? = when (tokenType) {
...
enum class ComparisonOp {
    ...
    fun invert(): ComparisonOp = when (this) {
    ...
    fun flip(): ComparisonOp = when (this) {
    ...
    companion object {
        fun fromJavaTokenType(tokenType: IElementType): ComparisonOp? = when (tokenType) {
