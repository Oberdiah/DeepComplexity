...
object ConstantExpression {
    ...
    fun <T : ConstrainedSetCollection<T>> fullSetFromExprAndKey(expr: IExpr<T>, key: Context.Key): T =
        expr.getSetIndicator().newFullSet(key)

    fun <T : ConstrainedSetCollection<T>> fullExprFromExprAndKey(expr: IExpr<T>, key: Context.Key): IExpr<T> =
        ConstExpr(fullSetFromExprAndKey(expr, key))

    fun <T : ConstrainedSetCollection<T>> emptySetFromExpr(expr: IExpr<T>): T =
        expr.getSetIndicator().newEmptySet()

    fun <T : ConstrainedSetCollection<T>> emptyExprFromExpr(expr: IExpr<T>): IExpr<T> =
