...
class Constraints private constructor(
    ...
    private val constraints: Map<Context.Key, ConstrainedSetCollection<*>>,
    ...
    companion object {
        fun constrainedBy(constraints: Map<Context.Key, ConstrainedSetCollection<*>>): Constraints {
    ...
    fun <T : ConstrainedSetCollection<T>> getConstraint(variable: VariableExpression<T>): T? {
        return constraints[variable.getKey().key]?.cast(variable.getSetIndicator())
    }

    fun <T : ConstrainedSetCollection<T>> getConstraint(ind: SetIndicator<T>, key: Context.Key): T? {
        return constraints[key]?.cast(ind)
    }

    fun addConstraint(key: Context.Key, expr: ConstrainedSetCollection<*>): Constraints {
    ...
    fun and(other: Constraints): Constraints {
        ...
        return Constraints(Functional.mergeMapsUnion(this.constraints, other.constraints) { lhs, rhs ->
            fun <T : ConstrainedSetCollection<T>> ugly(l: ConstrainedSetCollection<T>): ConstrainedSetCollection<T> =
