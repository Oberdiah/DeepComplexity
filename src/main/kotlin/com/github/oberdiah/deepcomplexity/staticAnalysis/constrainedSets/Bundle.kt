...
class Bundle<T : Any> private constructor(
    ...
    data class ConstrainedVariances<T : Any> private constructor(
        ...
        companion object {
            fun <T : Any> new(variances: Variances<T>, constraints: Constraints): ConstrainedVariances<T> {
                return ConstrainedVariances(variances.updateConstraints(constraints), constraints)
            }
        }
        ...
        fun reduceAndSimplify(scope: ExprEvaluate.Scope): ConstrainedVariances<T> {
            return ConstrainedVariances(
                variances.reduceAndSimplify(scope),
        ...
        fun toDebugString(): String {
            if (constraints.isUnconstrained()) {
                return variances.toDebugString()
            }
            val variancesStr = variances.toDebugString()
    ...
    fun performBinaryOperation(
        other: Bundle<T>,
        exprKey: Context.Key,
        op: (Variances<T>, Variances<T>) -> Variances<T>
    ): Bundle<T> =
    ...
    fun <Q : Any> binaryMapToVariances(
        ...
        other: Bundle<T>,
        exprKey: Context.Key,
        op: (Variances<T>, Variances<T>) -> Variances<Q>
    ): Bundle<Q> {
        ...
        for (myBundle in variances) {
            for (otherBundle in other.variances) {
                ...
                val newVariances = op(
                    myBundle.variances.updateConstraints(newConstraints),
                    otherBundle.variances.updateConstraints(newConstraints)
                )
    ...
    fun collapse(): ISet<T> {
        ...
        return variances.fold(ind.newEmptySet()) { acc, bundle ->
            acc.union(bundle.variances.collapse())
        }
    }
