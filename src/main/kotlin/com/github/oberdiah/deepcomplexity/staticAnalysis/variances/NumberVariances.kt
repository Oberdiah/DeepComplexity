...
data class NumberVariances<T : Number> private constructor(
    ...
    override fun toString(): String {
        return if (varsTracking().isEmpty()) {
            collapse(Constraints.completelyUnconstrained()).toString()
        } else {
            variances.entries
    ...
    fun updatedVariances(constraints: Constraints): Map<Context.Key, Variance<T>> {
        return variances.mapValues { (key, variance) ->
            Variance(
                ind,
                variance.multiplier,
    ...
    override fun toDebugString(constraints: Constraints): String =
        newFromVariances(ind, updatedVariances(constraints)).toString()
    ...
    override fun reduceAndSimplify(scope: ExprEvaluate.Scope, constraints: Constraints): Variances<T> {
        return newFromVariances(
            ind,
            updatedVariances(constraints).entries.associate { (key, variance) ->
    ...
    override fun collapse(constraints: Constraints): NumberSet<T> {
        val result = updatedVariances(constraints).entries.fold(ind.onlyZeroSet()) { acc, (_, variance) ->
            acc.add(variance.collapse())
        }
        return result
    }
    ...
    fun arithmeticOperation(
        ...
        when (operation) {
            ADDITION, SUBTRACTION -> {
                val me = updatedVariances(constraints)
                val other = other.updatedVariances(constraints)
            ...
            MULTIPLICATION -> {
                ...
                for ((key, meVariance) in updatedVariances(constraints)) {
                    for ((otherKey, otherVariance) in other.updatedVariances(constraints)) {
