...
// As part of that swapped FractionException for ArithmeticException so it compiled.
...
public final class BigFraction extends Number implements Comparable<BigFraction>, NativeOperators<BigFraction>, Serializable {
	...
	private static BigFraction from(final double value,
		...
		if (a0 > OVERFLOW) {
			throw new IllegalArgumentException("Value too large to be converted to a fraction: " + value);
		}
		...
		if (n >= maxIterations) {
			throw new IllegalArgumentException("Value did not converge after " + maxIterations + " iterations: " + value);
		}
